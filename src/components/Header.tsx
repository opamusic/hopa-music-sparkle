  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        {/* Desktop nav */}
        <nav className="hidden md:flex items-center gap-8" aria-label={lang === "he" ? "ניווט ראשי" : "Main navigation"}>
          {navLinks[lang].map((link) => {
            const isEnglishInHebrew = lang === "he" && /^[A-Za-z]/.test(link.label);
            return (
              <a
                key={link.href}
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href)}
                lang={isEnglishInHebrew ? "en" : undefined}
          ...
          <a
            ...
            className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${scrolled
                ...
                : "bg-black/40 text-primary-foreground backdrop-blur-sm border border-white/50 hover:bg-black/55"
              }`}
          >
            {ctaLabel}
          </a>
          <button
            ...
            className={`flex items-center gap-1.5 text-sm font-medium transition-colors duration-300 hover:text-primary ${scrolled ? "text-foreground" : "text-primary-foreground"
              }`}
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div id="mobile-nav" className="md:hidden glass mt-2 mx-4 rounded-2xl p-6 animate-scale-in">
          <nav className="flex flex-col gap-4" aria-label={lang === "he" ? "ניווט מובייל" : "Mobile navigation"}>
            {navLinks[lang].map((link) => {
              const isEnglishInHebrew = lang === "he" && /^[A-Za-z]/.test(link.label);
              return (
                <a
